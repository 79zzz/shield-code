...
from dataset1.new_semi import SemiDataset
...
from util.loss import CEWithEM
...
def main():
    ...
    if cfg['criterion']['name'] == 'CELoss':
        ...
        # criterion_l = CEWithEM(**cfg['criterion']['kwargs']).cuda(local_rank)
    ...
    # criterion_u = CEWithEM(reduction='none').cuda(local_rank)

    # 读取数据集 传入参数： name, root, mode, size=none, id_path=none, nsample=none
    ...
    previous_best = 0.0
    # previous_best_mpa = 0.0
    epoch = -1
    # 检查是否已经存在最新的模型检查点文件，如果存在的话，则加载这个检查点以恢复训练的状态。
    ...
    for epoch in range(epoch + 1, cfg['epochs']):
        ...
        for i, ((img_x, mask_x),
            ...
            with torch.no_grad():
                ...
                pred_u_w_mix = model(img_u_w_mix).detach()  # 创建一个张量，不影响计算 pred_u_w_mix类似于logits输出
                conf_u_w_mix = pred_u_w_mix.softmax(dim=1).max(dim=1)[0] # 第一个维度（类别）上进行softmax操作，并使用.max操作
                mask_u_w_mix = pred_u_w_mix.argmax(dim=1) # 返回每个样本对应的类别索引标号
            ...
            num_lb, num_ulb = img_x.shape[0], img_u_w.shape[0]   # 获取有标签和无标签数据的数据量
            ...
            pred_u_w = pred_u_w.detach()   # 类似于上面的操作
            ...
            if (i % (len(trainloader_u) // 8) == 0) and (rank == 0):
                logger.info('Iters: {:}, Total loss: {:.3f}, Loss x: {:.3f}, Loss s: {:.3f}, '
                            
                            'Mask ratio: {:.3f}'.format(i, total_loss.avg, total_loss_x.avg, total_loss_s.avg,
        ...
        if rank == 0:
            ...
            # 将MPA保存到日志文件中
            # logger.info('***** Evaluation {} ***** >>>> MPA: {:.2f}\n'.format(eval_mode, mpa))

            # 将MPA保存到TensorBoard中
            # writer.add_scalar('eval/MPA', mpa, epoch)
        ...
        # is_best_mpa = mpa > previous_best_mpa
        # previous_best_mpa = max(mpa, previous_best_mpa)
